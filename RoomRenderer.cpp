...
void RoomRenderer::Render(SDL_Renderer* pRenderer, const PlayerState& playerState, const WorldState& worldState, const Vector2d& baseTranslation)
{
	...
	for (unsigned int i = 0; i < worldState.Tiles.size(); ++i) 
	{
		...
		{
			SDL_Rect destRect = { 
				columnsRendered * (int)worldState.TilesSizeInPixels.X + (int)baseTranslation.X,
				rowsRendered * (int)worldState.TilesSizeInPixels.Y + (int)baseTranslation.Y,
				(int)worldState.TilesSizeInPixels.X,
				(int)worldState.TilesSizeInPixels.Y };
